...
public class EditFormView extends View {
	...
	protected EditFormView(AgendaEvent event) {
		this.event = event;
	...
	protected Node setupCenter() {
		...
		textFieldName.setText(this.event.getShortDescription());

		Label labelDate = new Label("Datum: ");
		DatePicker datePicker = new DatePicker();
		datePicker.setValue(this.event.getDate());
		...
		slider.setLowValue(this.event.getStartTime());
		slider.setHighValue(this.event.getEndDate());

		Button buttonCreate = new Button("Upravit");
		buttonCreate.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				// Odstrani stary predmet a nahradi jej novym zaznamem
				Main.getInstance().data.removeAgenda(event);
				...
				b.setHeaderText("Akce byla úspìšnì zmìnìna");
				b.setContentText("Akce byla úspìšnì upravena! Seznam akcí byl aktualizován!");
				b.showAndWait();

				AgendaView agView = AgendaView.getInstance();
				Main.getInstance().setScene(AgendaView.getInstance().setup());
				agView.editStage.close();
